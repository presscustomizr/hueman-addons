...
class HA_init {
    ...
    function ha_load() {
       ...
       *  Loads Features
      /* ------------------------------------------------------------------------- */
      load_template( dirname( __FILE__ ) . '/inc/sharrre/ha-sharrre.php' );
      new HA_Sharrre();
      load_template( dirname( __FILE__ ) . '/inc/shortcodes/ha-shortcodes.php' );
      new HA_Shortcodes();

      /* ------------------------------------------------------------------------- *
       *  Loads Customizer
      /* ------------------------------------------------------------------------- */
      load_template( dirname( __FILE__ ) . '/inc/czr/ha-czr.php' );
      new HA_Czr();
      ...
      /* ------------------------------------------------------------------------- */
      if ( HA_SKOP_ON ) {
        load_template( dirname( __FILE__ ) . '/inc/skop/init-skop.php' );
      }
      if ( HA_SEK_ON ) {
        load_template( dirname( __FILE__ ) . '/inc/sek/init-sektions.php' );
      }
    }
    ...
    */
    function ha_is_customizing() {
      ...
      if ( ! is_admin() && isset($_REQUEST['wp_customize']) )
        $bool = true;
      if ( $this -> ha_doing_customizer_ajax() )
        $bool = true;
      return $bool;
    }
